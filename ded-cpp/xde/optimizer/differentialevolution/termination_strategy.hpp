...
using termination_strategy = std::function<bool(individual_ptr, size_t, bool)>;
...
class max_gen_termination_strategy{
 ...
 public:
  ...
  bool operator()(individual_ptr best, size_t genCount, bool force_terminate) {
    return (genCount < m_maxGen) && !force_terminate;
  }
};
